...
allowed-tools: Read, Bash(git status *), Bash(git log *), Bash(node docs/check-docs.js *)
---

Orient me for this session. Read `docs/plan.md` → *Checkpoint* — it is the only place that says what is
next — then run `git status`, `git log --oneline -5`, and `node docs/check-docs.js`.
...
- anything `check-docs.js` failed on
