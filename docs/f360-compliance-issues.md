# Fusion 360 / Autodesk HSM Post-Processor Compliance Issues — MPCNC_v4.0_Beta1.cps
...
Legend: `[x]` resolved (either fixed in code, or reviewed and accepted **As designed** with no change), `[ ]` open.
...
- [x] **F1 — `tolerance` is unset but is used by every arc-linearization path.** — [Line 27](../MPCNC_v4.0_Beta1.cps#L27), [`circular()`: 1858-1928](../MPCNC_v4.0_Beta1.cps#L1858-L1928). **Fixed** by uncommenting line 27 so `tolerance = spatial(0.002, MM);` is assigned. Original finding: `tolerance = spatial(0.002, MM)` was **commented out**, yet `circular()` calls `linearize(tolerance)` in several places. Per Autodesk docs `tolerance` is a kernel-consumed global the post **must** assign (in MM), with **no documented default** and no documented kernel initialization (the class reference lists it as a bare `Number` attribute — unlike `unit`, which the docs explicitly say the kernel populates). An unassigned global reads as `undefined`, so these calls become `linearize(undefined)`.
  ...
  **Severity is latent-but-real.** In the common 2.5D case with "Use Arcs" enabled, XY arcs are emitted directly as G2/G3 and `linearize()` is never reached — which is likely why this hasn't been widely noticed. It manifests in two concrete paths:
  1. **`job4_UseArcs = false`** → [lines 1859-1861](../MPCNC_v4.0_Beta1.cps#L1859-L1861) route **every** arc through `linearize(tolerance)` = `linearize(undefined)`. This is a normal user choice (senders/controllers that handle arcs poorly), so it's a live trigger.
  2. **Non-XY-plane arcs on Marlin** (default firmware) and full-circle/helical fallbacks → the `linearize(tolerance)` calls in [circular()](../MPCNC_v4.0_Beta1.cps#L1858-L1928) (e.g. the Marlin branch linearizes ZX/YZ arcs, [1904-1927](../MPCNC_v4.0_Beta1.cps#L1904-L1927)).
...
- [x] **F4 — A non-default Work Coordinate System set in Fusion was silently ignored.** — Original: no `getWorkOffset` usage; the only WCS output was a hardcoded `G54` in the `toolChange1_InsertCode == false` branch — emitted for *all* firmwares including stock Marlin, which doesn't understand G54. With tool changes disabled (the default), the post emitted **no** work-offset code at all, so a Fusion setup assigning WCS 2+ (G55…) was silently posted against the single G92 origin.
  ...
  **Fixed** by adding a firmware-aware `writeWCS(section)` (reads `currentSection.getWorkOffset()`), called per-section in `onSection`, and removing the firmware-blind hardcoded `G54` from `toolChange`:
  ...
  - **Marlin** — keeps the existing `G92` origin ([`job1_SetOriginOnStart`](../MPCNC_v4.0_Beta1.cps#L1811-L1814)); emits **no** G5x (stock Marlin has none) and issues a warning comment when a non-default WCS (offset >1) is selected, since it can't be honored. This also removed the pre-existing bug where the hardcoded `G54` was sent to stock Marlin.
...
- [x] **F6 — Control-side radius (cutter) compensation is not supported; it hard-errors.** — [`onRadiusCompensation`](../MPCNC_v4.0_Beta1.cps#L1256-L1264), `linearMovements`. Erroring on unsupported control-side comp is a **sanctioned fallback** (Marlin/GRBL/RepRap have no G41/G42), so the behavior was already acceptable — but the error was generic ("Radius compensation mode is not supported.") and fired late (at the first compensated move in `linearMovements`). **Fixed** by rejecting it early in `onRadiusCompensation` with an actionable message that names the cause and the fix (set the operation's Compensation Type to "In computer"). Also removed the now-dead handling in `linearMovements`: the vestigial `xOutput.reset()/yOutput.reset()` block (which misleadingly implied comp was supported) and the duplicate "not supported" error — since `onRadiusCompensation` now halts before any compensated move reaches `linearMovements`. (Did not use `getSetting("supportsRadiusCompensation")` — that belongs to the newer settings-object post framework this older-style post doesn't use.)

- [x] **F7 — Coolant uses a bespoke dual-channel implementation instead of the declarative `coolants` array. — As designed (no change).** [`setCoolant`/`CoolantA`/`CoolantB`: 880-958](../MPCNC_v4.0_Beta1.cps#L880-L958). Autodesk's recommended approach is a declarative `coolant.coolants` array plus `setCoolant(tool.coolant)` / `getCoolantCodes()`, which the kernel validates. This post rolls its own two-channel, pin-based (M42 P# S#) coolant with custom-file overrides — a deliberate design to express MPCNC's pin-driven coolant, which the stock mechanism can't easily represent. **Accepted as designed:** the bespoke implementation is intentional; it only forgoes kernel-side coolant-mode validation, with no functional deficiency.

- [x] **F8 — No tool-length compensation output (G43 H#); tool-change block is `M6 T#`. — As designed (no change).** [`toolChange`](../MPCNC_v4.0_Beta1.cps#L1990-L2013). Industrial posts emit `G43 H#` with initial positioning. Marlin/GRBL/RepRap have no tool-length-offset tables, and this post instead re-probes Z after a tool change ([`probe2_OnToolChange`](../MPCNC_v4.0_Beta1.cps#L2000-L2010)). **Accepted as designed:** the G43 omission is correct for the target firmware (there are no tool tables to reference); Z is re-established by probing. (`M6 T#` order is accepted by these firmwares.)

- [x] **F9 — `maximumCircularSweep = toRad(180)` made the `isFullCircle()` branches in `circular()` dead code.** — [Line 34](../MPCNC_v4.0_Beta1.cps#L34), `circular()`. With a 180° max sweep, the kernel **splits** a full circle into two ≤180° arcs before `onCircular` (verified — it does not linearize), so full circles already posted correctly as arcs and the `isFullCircle()` handling could never execute; the nested `isHelical()` checks were also dead (`allowHelicalMoves = false` → the kernel linearizes helical moves before `onCircular`). **Fixed** by removing the dead `isFullCircle()`/`isHelical()` wrappers in both the GRBL and Marlin branches (keeping only the partial-arc switch that was already the sole live path — a behavior-preserving cleanup, no change to emitted G-code), and adding a comment explaining why `maximumCircularSweep` stays at 180° (splitting full circles into two arcs avoids the start==end full-circle quirks some firmwares mishandle). Chose this over setting `maximumCircularSweep = toRad(360)` (single-block full circles), which would be a behavior change with firmware-compat risk for marginal benefit.

- [x] **F10 — `getProperty(properties.key)` uses the object-reference form. — As designed (no change).** — throughout the file. Autodesk documents **both** `getProperty("key")` and `getProperty(properties.key)` (object reference) as valid; the object form is correct here because the post uses the **combined inline** `properties` structure (each key holds its full definition incl. `value`), which is exactly the form `getProperty(properties.key)` is designed for. **Accepted as designed — do not migrate.** The combined inline form is the deliberate, correct approach for this post: v3 was created specifically to move *to* it ("Updated to new method of handling properties"). The predecessor `MPCNC.cps` used the **split** `properties` + separate `propertyDefinitions` structure, which a Fusion change broke, forcing the reimplementation to the combined form. (Corroboration: code-quality bug #2 — a stray `propertyDefinitions.mapF_SafeZ.title` reference — was a leftover from that old split form.) So migrating to `propertyDefinitions` would be reverting to the very structure that broke; `getProperty(properties.key)` is correct and stays. *(Corrects an earlier draft of this finding that mischaracterized the split form as a newer/"modern" migration target — that was based on a source the review couldn't actually retrieve, and is contradicted by this repo's history.)*

- [x] **F11 — Multi-axis is only rejected at the first 5-axis move, not at section start.** — [`onLinear5D`/`onRapid5D`](../MPCNC_v4.0_Beta1.cps#L1248-L1258). Multi-axis toolpaths correctly `error()`'d out, but only once motion began — after the section header/comments (and possibly a tool change) were already emitted, with a message that didn't identify the operation. **Fixed** by adding a `currentSection.isMultiAxis()` guard at the top of `onSection` that errors before any section output, with an actionable message (use a 3-axis milling or 2D/jet strategy). The existing `onLinear5D`/`onRapid5D` errors are kept as a defensive backstop.

---

## Checked and NOT an issue (ruled out against the docs)

- **Formats built from `unit` at module-load time** ([lines 638-661](../MPCNC_v4.0_Beta1.cps#L638-L661)) — Autodesk confirms `unit` is populated **before** the global section executes, and the stock RS-274D sample computes `createFormat` decimals from `unit` at load time. Sanctioned pattern, not a bug.
- **`allowedCircularPlanes = undefined`** ([line 36](../MPCNC_v4.0_Beta1.cps#L36)) — documented to mean "allow all three planes" (not "disabled"). Correct.
...
- **`onPower` jet handling** ([`onPower`: 1265-1278](../MPCNC_v4.0_Beta1.cps#L1265-L1278)) — `onPower` is the correct/required jet on-off handler and is implemented; `currentSection.jetMode` (THROUGH/ETCHING/VAPORIZE) is read correctly. Compliant.
