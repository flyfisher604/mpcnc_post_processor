...
- **`A_Probe_BaseReserve`** (`None` default | `G54`–`G59` | `G59.1`–`G59.3 (RepRap)`).
...
- **`B_Probe_BaseEstablish`** (bool, default on): probe the spoilboard into the base at job
...
- **Guard A — no base redefine.** *Using* the reserved base is fine; an operation that would
  **re-establish its origin** (via `C_Probe_OnStart` / `D_Probe_OnChange` /
  `H_ToolChange_ProbeAfterChange`) errors.
- **Guard B — safe-Z across parts needs a base.** `J_Probe_SafeZAcrossWcs` on + >1 distinct
...
- **Group order** = the `group:` string, zero-padded to two digits (`01 - Job` …
  `10 - Duet`). Padding is required so `10 - Duet` sorts last, not next to `01 - Job`.
  ...
  > `Tool Changes`: `01 - Job`, `02 - Establish Machine Coordinates`, `03 - Feeds and Speeds`,
  > `04 - Map G1s to Rapids...`, `05 - Work Coordinate System - WCS / Probe`,
  > `06 - Tool Changes`, `07 …`. Renumber only the `group:` strings; item-letter keys are
  > unchanged. Update the README property-reference order too.
...
- `C_Probe_OnStart` = **"First Part: Set Work Origin"** — `Skip` / `Zero XYZ (no probe)` /
...
- `D_Probe_OnChange` = **"On Each Added Part"** — `Skip` / `Probe Z` (default). Fires on a
...
> **⚠ MARKED FOR REVIEW — make `B_Probe_BaseEstablish` an enum, not a toggle.** Relabel to
...
>    *zero-at-current-position*, parallel to `C_Probe_OnStart`, **not** machine homing (group
...
a **job-level clearance measured above the spoilboard base** (`K_Probe_SafeZClearance` =
...
1. Run `writeWCS()` first — it owns the base retract + frame switch. When a tool change on the
   same section will re-probe, have `writeWCS()` **skip its own `D_Probe_OnChange` probe** and
...
### Phase 4 — probe XY offset *(independent feature)*

Add two properties (X and Y) applied at **every** part probe — first (`C_Probe_OnStart`) and
each added part (`D_Probe_OnChange`). The probe touch-point becomes origin + (offsetX, offsetY)
instead of the raw origin, so the origin can sit at a corner / off the material while Z probes
the stock top. Job-wide (not per-fixture); default `0,0` → current behavior. Follows the
naming convention (next free item letters in the WCS/Probe group).

### Phase 4 — backlog: "Copy first part's Z" option on `D_Probe_OnChange`
...
`C_Probe_OnStart` time. Marlin no-op. The neutral "On Each Added Part" title already
...
- **`B_Probe_BaseEstablish`** defaults **on** (probe the spoilboard into the base); off emits
...
- **Multi-WCS is Replicate-only** — the per-copy Z re-probe (`D_Probe_OnChange`) and the
