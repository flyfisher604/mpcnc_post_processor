...
| The machine frame | group `4 - Machine Frame - homing and end park` (all three properties) — the declaration, the homing action, the end park |
| The fixed Z reference | group `5 - Fixed Z Reference - multi-part jobs only` (all five properties) — the spoilboard base and the machine-Z datum |
...
**✅ 11 fixed · ◑ 2 part-fixed · ⬜ 8 open — 21 findings.** The `HR-` ids were found by the hobbyist pass
...
**except PR-12, PR-13 and PR-14**, which the over-engineering assessment found and which continue the series
...
3. **Park position — two branches decided, and a third now sanctioned:** base reserved → park relative to
   ...
   "Never `G53`" that used to close this list is superseded** — see PR-4: with `Axes Homed and
...
| **PR-14** | The refusal chain the multi-WCS operator actually walks: Guard B, then the travel-height guard, then a file | **Multiple WCS Offsets** × 2 instances, GRBL, group 5 at defaults — then two edits | posted | §3.6 | ⬜ |
...
> **Settings note (Guard B) — rewritten 2026-08-13; the old advice cannot be followed.** It said to use
> **Retract Across Parts = Off** with no base. **CR-13 deleted that property and made Guard B
> unconditional**, so both halves are gone: there is no toggle, and *every* job with two distinct work
> offsets now needs a `Fixed Z Reference` — `Machine Z - homed` (no register spent, no base probe, needs
> `Axes Homed and Trusted` ⊇ Z and X/Y, `Home at Job Start` ≠ `Off`, and a measured `Inter Part Travel Z`)
> or `Spoilboard` with a `Reserved WCS`. **PA1 and PB2 therefore cannot be posted as written** — that is
> `09-plan.md` 1.3's first action, and it applies to this note too. What Guard B still cannot do is tell
> one part's two WCS from two fixtures; that complaint outlived its toggle and belongs to Step 1.2.
> The cost of *discovering* any of this from the dialog is **PR-14**.
...
- [ ] **D1 — labels, groups, field types, and the descriptions.** Confirm group **`6 - On WCS / Part /
      Fixture Changes`** exists (no lingering `Probe / Work Origin`); titles read **First WCS / Part**,
      ...
      **Added by the hobbyist dialog review** (`HReview.md` HR-29 · HR-30 · HR-31 · HR-32): group 4 reads
      `4 - Machine Frame - homing and end park`, group 5 `5 - Fixed Z Reference - multi-part jobs only`,
      and the group-4 declaration is `Axes Homed and Trusted`, not `Axises`. Then read the **leading
      sentence alone** of each of the eleven rewritten descriptions (the seven group-4/5/6 controls plus
      `jobManualSpindlePowerControl`, `probeThickness`, `probeG38Target`, `mapRapidsSafeZ`). **Pass — each
      answers *what do I set, and can I skip this*, unaided.** Merely shorter and still burying the answer
      is a fail. *(The only check those four findings can have: a title and a description exist nowhere
      but the dialog, so no posted file can evidence them.)*
- [ ] **D3 — the key rename resets every saved preset, once, and the new keys then hold.** Open the dialog
      ...
      `4 - Machine Frame - homing and end park`, `5 - Fixed Z Reference - multi-part jobs only`
      (both **renamed** by `HReview.md` HR-32), `6 - On WCS / Part / Fixture Changes`,
      `7 - Tool Changes`, `8 - External Include Files`,
      `9 - Laser`, `10 - Coolant`, `11 - Duet` — **in that order, none collapsed** — with
      **8 / 7 / 4 / 3 / 5 / 10 / 8 / 5 / 7 / 10 / 2** properties (**69** total), placed by `order:` (100…200 in
      ...
      **Pass — every property is back at its default, exactly once.** The key is the stored identifier and
      every key was renamed, so a customised Beta-1/Beta-2 preset **must** come up empty: a value that
...
- [ ] **D5 — the header property dump after the `groupDefinitions` move and the key rename.** Re-baselines
      ...
      **Pass — the diff touches only the eleven headings and the 68 property *names*; every *value* is
      ...
      post shows that Fusion's `getProperty()` resolves every renamed key.)*
...
- [ ] **PR-1a — the declaration emits what the old enum did.** *Do:* `Axes Homed and Trusted` through
      ...
      *Get:* GRBL one `$H` whenever either axis is declared, **identical for every declaration set**
      ...
      `( >>> WARNING: "Home at Job Start" is on but "Axes Homed and Trusted" is None …)` and no motion; the prompt appears once,
...
- [ ] **PR-14 — the refusal chain, three posts of one job.** *Do:* one F360 Setup with **Multiple WCS
      Offsets** ticked and **Number of Instances** 2, posted three times, changing one thing each time:
      (1) group 5 at factory defaults; (2) `Fixed Z Reference` = `Machine Z - homed`, nothing else;
      (3) plus `Inter Part Travel Z` = the absolute machine Z read off the DRO. *Get:* (1) Guard B,
      (2) the travel-height guard, (3) a file. **Pass: no `.gcode` on disk for 1 or 2** — both are
      `onOpen()` guards, so the discriminator is the *absence of the output file* — and step 3 posts.
      **Steps 1 and 3 ran 2026-08-13** with `Axes Homed and Trusted` = `XYZ` and `Home at Job Start` =
      `Home`. Step 1 returned Guard B's text verbatim, **CR-2's** homing-destroys-the-pre-jog warning (that
      one is `HReview.md`'s and is already posted there — not new evidence), and Fusion's own
      `Multiple work offsets used in program.`; **step 3 posted a file** once `Fixed Z Reference` =
      `Machine Z - homed` and `Inter Part Travel Z` were both set, raising **CR-2's again and PR-10's**, and
      nothing else. *The two are easy to confuse and must not be: CR-2's names the endstops, PR-10's names
      `Inter Part Travel Z`, and only the second one is this branch's.* **Step 2 was skipped** — the author went straight to a working configuration — so *the second
      refusal is still unposted*, and it is the leg that carries the finding. **The finding itself is
      confirmed by the operator regardless:** `[AUTHOR]`, on the successful post — *"the error message did
...
- [ ] **PR-7b — the dialog reads in the right order.** *Method `dialog`.* **Pass:** group 4 shows
      `Axes Homed and Trusted`, `Home at Job Start`, `At End Park At` in that order and nothing else;
...
> sketch. `design.md` → *Frames* holds the durable half.
...
1. **Every one of the 69 rows** — the figure here read `37` until 2026-08-13, a count left behind when §3
   absorbed the Beta-2 test plan; §3's own header has been right all along. §3.1 is the largest untested
   area in the post and needed a job nobody had built: a 2-copy Replicate or a two-Setup job. **That
   blocker is down.** `[AUTHOR]` built a multi-WCS job on 2026-08-13 from **Multiple WCS Offsets** — one
   Setup, a checkbox and an instance count, which is cheaper than either — and posted it (PR-14). Whether
   it substitutes for the rows written around *Replicate* specifically is a judgement at the keyboard;
   what it certainly gives is two distinct work offsets, which is what Guard B and most of §3.1 turn on.
