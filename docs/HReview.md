...
- **HB-15 (A)** — the error text ends at `or clear the field.` and claims nothing about truncation.
  ...
  nowhere else. Today's reading is `HB-7 (A).log` 23, both sentences present.
...
- **Every closed finding now has a passing posted artifact, and no unrun row belongs to one.** HB-2,
  HB-3, HB-4, HB-5, HB-7, HB-8 and HB-9 are all ✅, from eight files posted in one session off one build.
  The five rows still ⬜ — HB-11 through HB-15 — are each a row on an **open** finding, so not one of them
  can pass before its fix lands: **what this register owes now is code, not posts.** The open findings are
  proved by reading the post, and their firmware claims by firmware source, cited in each row.
