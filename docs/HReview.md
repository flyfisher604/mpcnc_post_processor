...
| **HB-20 (A)** | The one surviving control executes what all three executed when all three were on | The compared pair is *pre-fix, all three booleans `true`* against *post-fix, the one boolean `true`*. **No artifact, and the reason is a licence, not a shortcut:** exercising the mapper at all needs the F360 **Personal** edition, since that is what emits a travel as a `G1` in the first place, and the operator has no Personal licence now. Stubbing the `G0` input to fake one was offered and **declined** — it would prove the harness, not the post | **code walk** | ✅ |
...
- **HB-5 (A)/(B)/(C) — PASS, all three, both channels.** (A) the `format error` warning at line **1** of
  ...
  *it warns although the property is inert with the mapper off, which is 1770 looping both fields
...
- **HB-14 — PASS at both call sites, and the two posts are exact complements** — `15mm` on the probe field
  ...
  valid one, in **both** directions, which reads 1770's loop over both off artifacts. **Placement rather
  than wording attributes (B)** to 1140: `HB-14 (B).gcode` 146 sits inside `*** SECTION begin ***` where
...
- **HB-15 (A) — PASS at the dialog.** The text ends at `or clear the field.`, matching 1811-1813 word for
...
- **HB-20 (A) — PASS by code walk, and it is the one ✅ in this register with no artifact behind it.** Four
  reachable sites, walked with all three booleans `true` on the left and the one boolean `true` on the
  right. **`safeZforSection()` 1059 and `isSafeToRapid()` 1286** test the surviving property and are
  untouched, so `safeZHeight` resolves identically and both `zSafe` comparisons use the same threshold.
  **The branch chain 1331-1343 is value-identical, not merely equivalent:** `&&` returns its right operand
  when the left is truthy, so pre-fix `getProperty(allowZ) && zUp && xyConstant` *evaluated to* `zUp &&
  xyConstant` — the same boolean, not a coercion of it. **Short-circuiting cannot hide a difference here,
  which is the load-bearing half:** `zConstant`, `zUp`, `xyConstant` and `curZSafe` are `let`-bound at
  1309-1312 **above** the chain from pure `roundTo()` reads, `getCurrentPosition()` is called once at 1302,
  and the last `Debug` comment is at 1313 — so dropping a conjunct skips no computation, no call and no
  output. **`onLinear()` 2853** reduces on both sides to `if (forceSectionToStartWithRapid == true)`, since
  the property each side tests is `true` in the compared configuration, so the `else if
  (isSafeToRapid(…))` arm is reached in exactly the same cases and the whole dispatch matches. The only
  loss is one `getProperty()` read per branch, a pure accessor that emits nothing. **Two differences the
  walk does find, both outside the motion:** the group-3 dump loses its two lines — and keeps `master`
  before `safeZ`, because 3308 sorts by `order:` and 10 preceded 20 pre-fix too — and *if the Safe Z is
  malformed* the `format error` warning quotes the shorter title, in the file and at the dialog.
  **What this row does NOT claim, deliberately:** that no configuration changed. Two did, both named in
  the finding before the edit — master on with `Allow Rapid Z` off now converts Cases 2 and 3, and master
  off with first-rapid on has no successor. The walk is an identity proof for *one* pairing, and reading it
  as "the fold changed nothing" is the trap. **Corroboration available on any licence, and not owed:** the
  dialog's shape and the dump's two surviving lines are licence-independent, so the next post of any job
  shows both without a Personal seat.
...
- **Every arc closes.** All eight `G2`/`G3` blocks were checked by computing the centre from `I`/`J`
  ...
  rather than a modal one — `gPlaneModal`'s `onchange` calls `gMotionModal.reset()` (943), which is
...
- **It owes no post. Sixteen artifacts across two sessions ran twenty-two of the twenty-three live rows,
  and every one passed on first read** — no row was ever marked ❌, and none needed a second attempt to
  interpret. **One finding is still open, HB-10**, deferred to the professional pass by its own row because
  the dead property it names is group-7 work either way. *`HB-18 (A)` and `HB-19 (A)` were the only rows
  whose fix shipped ahead of them, and one re-post of `HB-17 (A)`'s job cleared both — what a register
  earns by naming, before the post, the exact string it expects to read afterwards.*
- **`HB-20 (A)` is the one ✅ with no artifact behind it, and the distinction is kept rather than blurred.**
  Its Method cell reads `code walk` so the two kinds of pass stay greppable, and the reason is structural:
  the mapper only has work to do on an F360 **Personal** job, because that edition is what emits a travel
  as a `G1`, and no Personal licence was available. **Stubbing the `G0` input was offered and declined**,
  on the ground that it would prove the harness rather than the post — the same objection that kept HB-13's
  relative lift out of the code. What replaces it is an identity walk over four sites, and its strength is
  that the two Z branches were pure conjunct-drops over eagerly-bound locals, so the equality is decidable
  by reading rather than by sampling. **The dialog and dump halves need no Personal seat**, so the next post
  of any job corroborates them for free; that is noted as available, not as owed.
...
- **The line numbers cited here had already drifted, and each pass renumbers only what it moved itself.**
  The HB-18 / HB-19 commit displaced thirteen citations and HB-20 displaced nineteen more; both sets were
  renumbered with the commit that moved them and **every one re-verified as `old@n` in `git show HEAD:` to
  `new@n+d`**, which is the check that matters — a shift applied to a wrong number yields another wrong
  number. HB-20's pass found one that way: **HB-18's `writeBaseEstablish()` citation was a genuine
  mis-citation**, not a drift, pointing at the sibling `Fixed Z Reference = Machine Z` warning 26 lines
  off; its row quotes the reserved-base string verbatim, so it was corrected outright rather than shifted.
  **The older drift underneath is now confirmed rather than suspected, and still deliberately unfixed** —
  CR-2's `(homedXY || homedZ)` test is at 1617 where three rows say 1580, its `!homedXY && !homedZ`
  complement at 1605 where one says 1568, HB-11's two blank-separator sites at neither 2671 nor 3345, and
  `parseProbeSafeZProperty()` (1233), `onSectionEnd()` (2784), `Start()` (4093), `circular()` (4236) and
  `askUser()` (4309) each cited short by tens to hundreds of lines. All sit in closed rows, so nothing live
  reads them, and they want **one sweep** rather than a piecemeal ride on whichever commit happens to touch
  the file — that was the recorded decision and HB-20 kept it. **Until that sweep, a cited number is a hint
  and not evidence**, and a row that cannot be re-found is a row that cannot be re-checked.
- **HB-13 (b) has an answer, and the baseline artifact shows it working.** `HB-2 (A).gcode` 125-127 is the
  ...
  *alarming*; withheld because 3758-3762 records the decision and group 6 belongs to another pass.
