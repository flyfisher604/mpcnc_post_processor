...
**✅ 12 PASS · ❌ 0 FAIL · ⬜ 7 UNRUN · ➖ 3 n/a — 22 rows.**
...
| **HB-16 (A)** | The post's own `G17` is not suppressed by what an include file did to the plane | HP-1 + `Start GCode File` = a header whose only g-code is `G18`, on `HB-2 (A)`'s job. Today's post is the pre-fix reading | posted | ⬜ |
...
- **HB-16 (A)** — the first `G2`/`G3` after the loaded header carries its own `G17`. The discriminator
  is presence on **that** arc: read 159's XY arc and not a later one, since every arc after the first
  `G18` one re-asserts XY unaided and would pass vacuously. Today's file has `G17` only in `Start()`'s
  block, so the header's `G18` stands into the cut. Nothing in one post tests HB-16's cross-file half:
  that needs two posts in one Fusion session *and* a Fusion that reuses the JavaScript context, and the
  one-`G90`-per-file count says this one does not.
...
- **Twelve findings are closed, and five of them owe a post rather than code.** HB-2, HB-3, HB-4, HB-5,
  HB-7, HB-8 and HB-9 are ✅ from eight files posted in one session off one build. **HB-11, HB-12, HB-13,
  HB-15 and HB-17 are fixed but unposted** — the state HB-7 to HB-9 were in before this session, and what
  the dating bullet below bounds. HB-11 and HB-15 are re-posts of existing jobs; HB-12 (A) and HB-16 (A)
  each need a small include file first, one of which could serve both; HB-13 (A) is one configuration at
  two comment levels; HB-17 (A) is this register's only **Marlin** post. HB-14 and HB-16's include half
  are still **open**, proved by reading the post and their firmware claims by firmware source, per row.
- **HB-13 (b) has an answer this register is not taking.** A provisional `G10 L20 P<wcs> Z0` at the start
  height would make `G38 Target` a relative limit here as it already is on `Current XY & Probe Z`, on
  the same promise the warning states, trading an unbounded descent measured from a stale register for
  one that fails by *alarming*. Withheld because 3686-3691 records that decision where the probe starts
  from a clearance, and the trade belongs to whichever pass owns group 6.
