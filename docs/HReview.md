...
## Findings — HB-1 … HB-16 — 16 findings
...
**✅ 12 PASS · ❌ 0 FAIL · ⬜ 6 UNRUN · ➖ 3 n/a — 21 rows.**
...
- **Ten findings are closed, and three of them owe a post rather than code.** HB-2, HB-3, HB-4, HB-5,
  HB-7, HB-8 and HB-9 are ✅ from eight files posted in one session off one build. **HB-11, HB-12 and
  HB-15 are fixed but unposted** — exactly the state HB-7 to HB-9 were in before this session, and the
  state the dating bullet below exists to bound. HB-11 and HB-15 are the two cheapest, being re-posts of
  jobs that already exist; HB-12 (A) and HB-16 (A) each need a small include file written first, and one
  of those files could serve both if the footer and the header are posted separately. HB-13, HB-14 and
  HB-16's include half are still **open**, so their rows cannot pass before a fix lands. The open
  findings are proved by reading the post, and their firmware claims by firmware source, cited in each row.
- **Three of the sixteen findings were discovered by posting rather than by reading — HB-13, HB-14 and
