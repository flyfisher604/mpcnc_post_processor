...
| `PReview.md` — **in the tree**, and findings are filed against it directly. It was uncommitted from `7243869` to `d010fee` and reachable only in git; that arrangement cost the project the visibility of its own open questions and is not to be repeated (`CLAUDE.md` → *Registers ship with the code*). Content: PR-1…PR-11, seven `HR-` ids, §2–§6, 65 test rows | Professional findings, professional test rows, **unbuilt design and its open questions** (§6), the jet/laser workstream | Hobbyist-only findings, durable conventions | **≤ 920 lines, and falling.** §2's long form and §3's expansions are unbuilt design and unrun rows; both retire on build. The §3 index table is permanent, everything under it is not |
