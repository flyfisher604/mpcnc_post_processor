...
**200 cases — 37 hobbyist, 46 professional, 39 WCS, 11 personal, 51 CorrectGcode, 16 GCodeStructure —
over 42 job files, and all 200 pass as of 2026-08-20.** The whole run is under a minute.
...
|---|---|---|
| `jobSpindleControl` = `3` | `CG32` | `M3 S5000` and `M5` **replace** the two prompts — both halves, because a case that asserted only the `M3` would pass on a post that commanded the spindle *and* still stopped to ask |
