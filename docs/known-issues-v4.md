...
- [ ] **#8 — Severity: Correctness** — [Line 2043](../MPCNC_v3.0_Beta3.cps#L2043) — `rapidMovementsZ(...)` called with a stray second argument (`false`); the function only takes one parameter (`_z`), so it's silently dropped — possible remnant of an incomplete refactor.
- [ ] **#9 — Severity: Correctness** — [Line 719](../MPCNC_v3.0_Beta3.cps#L719) — Typo `alue: 2` instead of `value: 2` in `eSafeZ.prop[2]`. Currently unused elsewhere, but a landmine if ever referenced.
